...
def get_default_network_config(dim=2, dropout_p=None, nonlin="LeakyReLU", norm_type="bn"):
    ...
    # old version
    props['conv_op_kwargs'] = {'stride': 1, 'dilation': 1, 'bias': True}  # old model kernel size will be set by network!
    ...
    def __init__(self, previous, num_classes, num_blocks_per_stage=None, network_props=None, deep_supervision=False,
        ...
        for i, s in enumerate(np.arange(num_stages)[::-1]):
            ...
            if deep_supervision and s != 0:
                ...
                # old version
                seg_layer = self.props['conv_op'](features_skip, num_classes, 1, 1, 0, 1, 1, bias=False) 
                if upscale_logits:
        ...
        # old version
        self.segmentation_output = self.props['conv_op'](features_skip, num_classes, 1, 1, 0, 1, 1, bias=False)
